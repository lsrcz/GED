...
#include "matop.hpp"
#include "genCost.hpp"
void solveQuadratic(costMat c, double* x, double* b, double& s_k, double& l, double* mult_x_delta){
	...
	scalarMulVec(mult_x_delta, c.d_n, 2, mult_x_d);
	...
	if (beta <= 0 || t_0 >= 1){
		...
		double tmp = 1 - t_0;
		double* tmp_vec1 = (double*) calloc(c.d_n, sizeof(double));
		double* tmp_vec2 = (double*) calloc(c.d_n, sizeof(double));
